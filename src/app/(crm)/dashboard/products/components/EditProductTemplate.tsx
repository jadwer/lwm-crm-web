...
const formatDecimal = (value: string): string => {
  ...
  return int + "." + dec.padEnd(2, "0").substring(0, 2);
};
...
const EditProductTemplate = ({ producto }: { producto: Product }) => {
  ...
  const [cost, setCost] = useState(() => formatDecimal(producto.cost?.toString() ?? "0"));
  const [price, setPrice] = useState(() => formatDecimal(producto.price?.toString() ?? "0"));
  ...
  const submitProductUpdate = async (event: MouseEvent<HTMLButtonElement>) => {
    event.preventDefault();

    const formData = new FormData();
    formData.append("name", nombre);
    formData.append("sku", sku);
    formData.append("description", descripcion || nombre);
    formData.append("full_description", descripcionTecnica || nombre);
    formData.append("cost", formatDecimal(cost));
    formData.append("price", formatDecimal(price));
    formData.append("iva", iva);
    formData.append("unit_id", unidad?.toString());
    formData.append("category_id", categoria?.toString());
    formData.append("brand_id", marca?.toString());
      ...
      <div className="container-fluid container-product">
        <form className="row g-3">
          <div className="col-12 col-md-4">
            ...
            {selectedImage && <Image src={selectedImage} width={300} height={300} alt="Vista previa" className="img-fluid" />}
          ...
          <div className="col-12 col-md-8">
            <div className="row">
              ...
              <div className="col-md-6">
                ...
                <input className="form-control" value={sku} onChange={(e) => setSku(e.target.value.replace(/\s+/g, "-"))} />
              </div>
              ...
              <div className="col-md-6">
                ...
                <input className="form-control" value={cost} onChange={(e) => setCost(e.target.value)} />
              </div>
              ...
              <div className="col-md-6 mt-2">
                <input id="iva" type="checkbox" className="form-check-input" checked={iva === "1"} onChange={(e) => setIva(e.target.checked ? "1" : "0")} />
                <label htmlFor="iva" className="form-check-label ms-2">Incluye IVA</label>
              </div>

              <div className="col-md-12">
                <label className="form-label">Ficha técnica (PDF)</label>
                <input type="file" accept="application/pdf" onChange={(e) => setDatasheet(e.target.files?.[0])} />
                {producto.datasheet_path && (
                  <div className="d-grid gap-2">
                    <a href={`${process.env.NEXT_PUBLIC_BACKEND_URL}/storage/datasheets/${producto.datasheet_path}`} target="_blank" rel="noopener noreferrer" className="btn btn-secondary mt-1">
          ...
          <div className="col-12">
            ...
            <textarea className="form-control" value={descripcionTecnica} onChange={(e) => setDescripcionTecnica(e.target.value)} />
            <br />
            <Link href="/dashboard/products" className="btn btn-secondary me-4">Cancelar</Link>
            <button type="button" className="btn btn-primary" onClick={submitProductUpdate}>Guardar</button>
          </div>
        </form>
      </div>
    </main>
